...
%%%%% genrating synthetic distributions under 625 parameter sets of positive feedback model

function positive_N100000
...
global  xdata tdata tt  mean  fano %%数据


lat11=[0.3,0.7,1,2,4];lat22 =[0.05,0.1,0.5,1,1.5]; gatt =[0.3,0.7,1,2,4];  v=[10,15,20,25,30]; 
...
filename='N100000_positive_10%.xlsx';  %%%% SSA for 100000 cell samples with 10% extrinsic noise on synthesis rate.
...
for i=2:M  % number
    p1(ta,i)=p1(ta-1,i)+dt*((gaT+(i-1)*nu)*p2(ta-1,i)-((i-1)*(del+mu)+la1)*p1(ta-1,i)+i*del*p1(ta-1,i+1));
    p2(ta,i)=p2(ta-1,i)+dt*((la1+(i-1)*mu)*p1(ta-1,i)-(vT+(i-1)*(del+nu)+gaT)*p2(ta-1,i)+i*del*p2(ta-1,i+1)+vT*p2(ta-1,i-1));
end
...
for i = 1:N
 s(1)=1;
  ...
  while t(n) <= T
        ...
        h3 = 1; h4 = X(n); c3 = lam ; a3 = h3*c3+h4*mu;  % I1 --> ON 
